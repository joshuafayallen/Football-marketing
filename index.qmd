...
The next step we are going to define a spline for the season component. Normally, I would use a Hilbert-Space Gaussian process for the time component, so it is worth mentioning the strengths and weakness of both. Briefly a Gaussian process (GP) genaralizes the multivariate normal distribution to infinite dimensions. It is a collection of random variable where any finite subset of random variables have a Gaussian distribution. If you are confused than do not be ashamed. I have yet to find a definition of GP where I was more informed about the Gaussian process. Basically a GP lets place a prior on an entire curve rather than individual data points. Lets take age as our working example since it is a bit more intuitive. Age effects lots of different phenomena and those effects are, generally, non-linear. We can make ad-hoc decisions about that relationship but a lot of the times unsatisfying. GPs generally solve this problem albeit slowly. 
...
So what is a HSGP? Effectively a HSGP is a just an approximation of GP. The only real drawback of GPs are that they are very slow. Any dataset of moderate size will slow down a GP pretty significantly making the process of building a model prohibitively slow. A HSGP using a lot of basis functions to approximate the full Gaussian process. This converts the most expensive operations like inverting a matrix into much quicker operations. 

However, for a lot of the play callers that we are interested in like Kyle Shanahan, Andy Reid, Sean Payton, and Sean McVay we never observe the full tenure of their play calling. The participation data that we are using only goes back to 2017 so we can't get any information about their personel usage. So most of what the HSGP is going to learn comes from mid-tenure coaches. This means the values closer to early tenure values are going to more heavily influenced by the prior, or what I think we would expect the carryover of the scheme some previous seasons would be. We aren't capturing the effect of say the Atlanta Falcons when the Shanahan scheme is starting to gain traction and really attack the Seattle Seahawks cover 3 style of defenses.

If we wanted to make the HSGP more meaninful we have to introduce more and more play callers, but they do not neccessarily have enough data to derive meaningful estimates of personnel contribution. Substantively I am also not neccessarily interested in how the personnel usage of offensive play callers who call plays for a year or two and are fired for a variety of reasons.^[They would be if anything an interesting foil for more succesful play callers] It would be interesting to see how personnel usage has varied for say Andy Reid and Sean Payton because they cut their teeth in several different eras in the NFL. I don't find it particularly interesting to look at Matt Patricia's use of personnel groupings and how they impacted his explosive play rate. 

To get around this problem I am going to use a spline. A spline is really just piecewise polynomial that we partition in a number of disjoint regions. The boundaries of these regions are defined by a vector of k knots that we order to construct an interval between each successive knot. Each of these regions have their own polynomial and we can define the full spline as a function of these distinct polynomials. One trick we can use is that any spline can be rewritten as a weighted sum of basis splines or B-splines. B-splines sound somewhat complicated but Richard McElreath gives some good intutition of this.  
...
The reason I prefer HSGPs for these kinds of problems is that we don't have a ton of ad-hoc decisions we need to make. We really just go through a Bayesian workflow to set our priors, check whether they are reasonable, and rinse and repeat. Whereas splines require a lot more decisions that are a bit harder to justify like the degree of polynomial we are using, how many knots to use, and then on top of that set some priors. However, splines are generally preferable to some other ad hoc approaches available to us. 
...
Then we are going to generate the models 

```{python}
#| code-fold: true

n_knots = 19
knots = np.quantile(cherries['year'], np.linspace(0,1, n_knots))
spline = dmatrix(
    "bs(year, knots = knots, degree = 3, include_intercept=True)-1", 
    {'year': cherries['year'], "knots": knots[1:-1]}
)
...
with pm.Model() as spline_mod:
    ...
    beta = pm.Normal('beta', 0, 1, shape = basis_set.shape[1])
...
As we can see the HSGP does a really great job of  fitting the cherry blossom dataset while the spline does a reasonable job of fitting the dataset. The main 
...
Then we are just going to fuse everything together by taking our saturated x's and our prior on the channel contributions. In this case, I separated out the pass rate from the rest of the controls just because I want it to vary a bit more. I ended up doing this just because it seemed to improve sampling and the overall fit of the posterior predictive. 
...
Let’s look at the personnel contributions, control contributions, and our mu parameter. Again, these are looking good. 
...
In this case the model does an okay job at fitting the underlying data, but we miss a lot of the interesting shapes and bumps in the observed data. Let’s just look at the contributions per channel. 
...
We don't see any appreciable returns for personnel groupings. Obviously, this could be true! The design of the play is probably doing more than putting an extra tight end on the field. But it could be that the model is not capturing the underlying data well! We have seen that the posterior predictive sort of follows the underlying data. We assumed that the saturation curve was logistic meaning there is some theoretical inflection point where we shouldn't spend any more money on that channel. More games are going to have fewer explosive plays than be closer to 25% of plays being explosive, so the model needs to handle the bunching closer to zero. Additionally, we defined investment in terms of share of plays rather than the number of plays. 

The other thing that can be said is that we don't really have a ton of data for each coach. While Andy Reid has called plays for 18 years, we only observe 8, which equates to 144 weeks of data.^[In the latter parts of the 2017 season, Andy Reid handed play-calling duties to Matt Nagy] This is right around the recommended amount of data for a reliable MMM. Partially, this is because we want the model to learn the features of the data and not have our priors dominate. The other part is that we have a ton of parameters for a kind of small model. For our channel contributions, we have around 5 parameters without accounting for hierarchies and our HSGPs. It may be that we don't need the HSGP for the season because it is eating some of the variation that the channels should be explaining.

### Refining the model 

Through lots of fiddling, I found that the only real way to improve the fit of the model is to change the likelihood. The first thing I tried, since I have never worked with one, was a mixture model. The main idea is that populations are, generally, made up of a lot of different kinds of groups. For example, if we plotted all the heights in the NFL, we would get something that looks pretty close to a Gaussian distribution. However, we know that teams generally use different height thresholds for each position group. Consequently, the overall distribution of heights in the NFL is just a mixture of the smaller distributions of heights in position groups. 
...
We can use this idea to either model these distributions, essentially, creating a bunch of component Gaussian distributions with a different mean and maybe a different standard deviation. Then we are going to model each of those components and combine them at the end. It does this by assigning a weight to each component part to indicate where it is likely to belong. Mixture models are not necessarily limited to models with distinct sub-populations. We can use these models to give us flexibility for bimodal distributions, phenomena where we have lots of zeros or ones, and a whole host of other things. 

For example, if we wanted to model whether a player is likely to be drafted in the first round and then their projected value as a first-round pick, we would want to build a hurdle model. We can think of a hurdle model as a mixture model with a particular structure where we are modeling whether they are above some threshold and then modeling another data-generating process. So, in our example, the hurdle model is going to estimate the probability they would be picked in the first round. The model that handles whether they were a first-rounder would be a simple binary classifier where we include variables like college production, their measurables, some observable measure of their character, and the quality of their competition. The next part of the model would then just model the expected value of the pick. Mixture models give us lots of ways to build flexibility into our models. 

In our case, there are a bunch of different dynamics that could lead to an offense having no explosive plays in a game. Additionally, player availability during the season could cause bimodality in the data that we aren't modeling. We saw in @fig-ppc-simple-beta that there were lots of peaks and valleys as well as lots of bunching closer to zero.^[Undoubtedly this is partially because of our clipping procedure.] By using a mixture model, we can try to handle some of the quirks of this data by dividing the model a few different ways. 

The first way I tried was to keep passes and rushes together and then try to separate the games with low-explosive plays. This could be some combination of a bad game, terrible weather, or the defense playing an insane game that could explain low explosive play production. Just as likely is that over the course of the NFL season, players are going to get hurt. This is going to have big impacts as you dip into offensive line depth or your third wide receiver is now getting all the targets that would be going to your WR 1. There are also things that can vault you into the middle of the distribution, like a rookie WR finally figuring out how to play in the pros, or you are going against an easier stretch of your schedule. 
...
The most important part is to define the mixture part. Here, we are going to try to learn whether each coach is in a near-zero play regime. We could really make this complicated by adding more covariates to our `w_coach_logit` param. However, just using the baseline for each coach gets us pretty close. The posterior predictive follows our observed data a lot better. 
...
At this point in the journey, we stick it in the file drawer and find a different project. But I am inherently stuborn so we are going to try again. What seems to work well is trying to model the low to no explosive play rate regimes, the mean explosive play rate regimes, and the high explosive play rate regimes separtely. To get around the problem of fitting 2-3 separate models for each regime we can try to consolidate this into one likelihood. 

To do this we are going to fit an Ordered Beta Regression. The main idea is that we are going to use a Dirichlet distribution and a Beta distribution fused together. What the model effecitively does is treat your outcome as a fusion of three categories: exactly 0, between 0 and 1, and exactly 1. Our model is really just a fancy ordered logit where the cutpoints between categories determines the shifts in probabilities between categories. 

Borrowing some work from [Andrew Heiss's blog post](https://www.andrewheiss.com/blog/2023/09/18/understanding-dirichlet-beta-intuition/) we 

```{python}

draws = rng.dirichlet(
    alpha = [3,7,2], size = 100
)
...
dat = (
    ...
    .with_columns(
        pl.when(
            pl.col('variable') == 'column_0'
        )
        .then(pl.lit('Category A'))
        .when(
            pl.col('variable') == 'column_1'
        )
        .then(pl.lit('Category B'))
        .otherwise(pl.lit('Category C'))
        .alias('nice_cat')
    )
)
fig, axe = plt.subplots()
g = sns.FacetGrid(dat, col = 'nice_cat', hue = 'nice_cat')

g.map(sns.kdeplot, 'value')
g.set_titles(col_template="{col_name}")
g.axes[0,0].set_xlabel('Probability of Proportion')
