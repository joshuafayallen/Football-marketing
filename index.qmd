...
execute: 
  ...
  knitr:
    opts_chunk:
        fig_pos: "H"
draft: false
description: "Modeling ROI of different personnel packages"
tags: 
 - Marketing
 - Bayesian Statistics
 - MMM
slug: /blog/2025/nfl-marketing/
showinProjects: true
---

::: {.callout-important}
This project is still [under very active development]{.underline}. If you spot any obvious errors or have some suggestions [please open an issue!](https://github.com/joshuafayallen/Fantasy-Football-Project/issues)
:::
...
# make sure that reticulate doesn't do anything crazy
library(arrow)
...
```{python}
#| code-fold: true
#| label: visualize explosive_play_rate
...
```{python}
#| label: plot-explosive-play-rate
...
comps = (
        pl.read_parquet('blog-post-data/comps.parquet')
        .rename({'__index_level_0__': 'mod'})
)


clean = (
    comps
    .select(pl.col('mod').alias(''), pl.all().exclude('mod'))
) 
...
As we can see the model does a much better job of predicting the observed data! 
...
add_personnel_contributions.write_parquet(
    'contribution',
    pyarrow_options = {"partition_cols": ['coach', 'personnel_grouping']},
    use_pyarrow = True)
```


Then from there we can just bring everything into R. The interesting thing about the Polars parquet writer native parquet writer is that it creates some inconsistencies in the strings. So we do have to resolve them later in R. 
...
raw_data = open_dataset('contribution', format =  'parquet', partitioning = c("coach", "personnel_grouping"), schema = scheme) |>
  collect()
...
make_marginal_lift = raw_data |>
  mutate(
    ...
    personnel_nice = str_remove(personnel_grouping, 'personnel_'), 
    personnel_nice = as.factor(glue::glue("{personnel_nice} Personnel")), 

  ) |>
    filter(coach %in% c('Kyle Shanahan', 'Sean McVay', 'Andy Reid', 'Sean Payton'))

```


## The Returns on Personnel Groupings

So after an admittedly long-winded section on what I did I think we should try to learn something about football. Instead of focusing on every play caller in the dataset I am going to subset the data to only include Kyle Shanahan, Sean McVay, Andy Reid, and Sean Payton.^[This just means I am dropping Josh McDaniels, Matt LaFleur, Zac Taylor, Kellen Moore, and Arthur Smith] I these four the most interesting due to their reputations and in some ways represent their respective eras of offense/philosophies quite well.


To give you a sense about what these plots are saying on average teams run anywhere between 60-70 plays per game. So if a team has about 60 plays a game and their marginal lift was 0.005 they would have, roughly, `r 60 * 0.05` explosive plays per game or about an extra explosive every 3 games. At the top I will note we don't see a huge return for any personnel groupings. While I didn't expect huge effects I was still struck by how small the median effects were and how wide the intervals were. I will return to why I suspect this is the case but for now I will just focus on the substance of the findings. 

Kind of unsurprisingly, 11 personnel leads all other personnel groupings in terms of marginal left. Intuitively this makes the most sense. In 11 personnel you have three WRs on the field making it much more amenable for passing and for run concepts if your WRs are good blockers you can leverage that against the defense for explosive runs. A pet cat of analytics for the last 10ish years has been that teams should be passing more because it is more efficient and more effective than passing the ball. 
...
Initially I was kind of surprised that 21 personnel had the second highest relative lift. This personnel grouping is a hallmark of classic "3 yards and cloud of dust" football or really conservative run heavy football that is meant to grind you down throughout the game. No doubt that some of this is due to the fact that Kyle Shanahan consistently has really explosive offenses and also employs 21 personnel at or near the top of the league. However, this is also structural to how modern offenses work. 

Ben Solak does a much more in depth schematic deep dive on these offenses in this [article](https://www.theringer.com/2024/02/07/nfl/super-bowl-preview-kyle-shanahan-offense-evolution-san-francisco-49ers). To paraphrase Ben pretty heavily how the Shanahan and McVay systems are so effective is how use they use these personnel groupings to move defenders to where they want and then setup the play around where they expect them to be. Take this example from Ben's article. Due to the fact that the Niners are in 21 personnel with Christian McCaffrey and Kyle Juszczyk the Lions are thinking run. The Niners use the combination of motion and keeping Juszczyk and Kittle in to block to free up the middle of the field to hit Deebo. 



<iframe width="560" height="315" src="https://www.youtube.com/embed/xWGrhk9CJsY?si=_awpnqolNKsfqzZP" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

We can see a similar thing happening early on in Shanahan's tenure out of 11 personnel. Here Deebo Samuel lines up in the backfield with Matt Breida. If you look at the second level of the defense you see two defensive backs, Nickell Robey-Coleman and Taylor Rapp, lined up alongside a linebacker. This forces the DBs to play out of a spot they are likely not comfortable with. The ideal outcome for the Niners is that Deebo outruns the corner. However, there is also a world where you are forcing the DB to take on a block from an offensive lineman which every coordinator and lineman would take in a heartbeat. 
...
sf_colors = c(nflplotR:::primary_colors['SF'])
ar_colors = c(nflplotR:::primary_colors['AR'])
ball = '#964B00'
...
sf_run = read_tsv('https://raw.githubusercontent.com/asonty/ngs_highlights/refs/heads/master/play_data/2019_SF_2019122102_1184.tsv')  |>
    mutate(teamAbbr = ifelse(is.na(teamAbbr), 'Ball', teamAbbr),
            colors = case_when(teamAbbr == 'SF' ~ sf_colors, 
                                teamAbbr == 'LA' ~ la_colors,
                                .default= ball))

y_min_run = max(round(min(sf_run$x, na.rm = TRUE) - 10, -1), 0)
y_max_run = min(round(max(sf_run$x, na.rm = TRUE) + 10, -1), 120)


df_hash <- expand.grid(x = c(0, 23.36667, 29.96667, xmax), y = (10:110))
df_hash <- df_hash |> filter(!(floor(y %% 5) == 0))
df_hash <- df_hash |> filter(y < y_max_run, y > y_min_run)

df_hash_run = df_hash |>
    filter(y < y_max_run, y > y_min_run)
...
scale_size_manual(values = c(6, 4, 6), guide = FALSE) +
  ...
  scale_fill_manual(
    values = c(sf_colors, "#654321", la_colors),
    guide = FALSE
  ) +
  scale_colour_manual(
    values = c("black", "#654321", sf_colors),
    guide = FALSE
  ) +
  annotate(
    "text",
    x = df_hash_run$x[df_hash_run$x < 55 / 2],
    y = df_hash_run$y[df_hash_run$x < 55 / 2],
    label = "_",
    hjust = 0,
    vjust = -0.2
  ) +
  ...
  annotate(
    "segment",
    x = xmin,
    y = seq(max(10, y_min_run), min(y_max_run, 110), by = 5),
    xend = xmax,
    yend = seq(max(10, y_min_run), min(y_max_run, 110), by = 5)
  ) +
  annotate(
    "text",
    x = rep(hash.left, 11),
  ...
  annotate(
    ...
    y = c(y_min_run, y_max_run, y_max_run, y_min_run),
    xend = c(xmin, xmax, xmax, xmin),
    yend = c(y_max_run, y_max_run, y_min_run, y_min_run),
    colour = "black"
  ) +
  geom_point(
    data = sf_run,
...
animate(anim_rush, nframes = 300 ,fps =  50)



```

So structully it makes sense that one of the most used personnel groupings by an offensive play caller that consistently has top 10 offenses is, generally, pretty explosive. 

If we look at who is extracting the most out of each personnel grouping an intersting story emerges. McVay has a very tight distribution and much tighter HDI's for 11 personnel while Andy Reid has a much higher median and wider distribution. We see a similar trend emerge with 21 personnel with Shanahan and Payton albeit the difference is much more muted between the two play callers. 

```{r}
formations_subset = make_marginal_lift |>
  filter(personnel_grouping %in% c('personnel_11', 'personnel_21'))
...
ggplot(
  ...
  scale_fill_met_d(name = 'Lakota') + 
  scale_color_met_d(name = 'Lakota') +
  facet_wrap(vars(personnel_nice), scales = 'free') + 
  labs(y = NULL) + 
  theme_minimal() + 
  theme(legend.position = 'none')
...
Part of the story behind this is structural. One of the reasons why the intervals are so tight for Shanahan and McVay is that they run most of their plays out of 21 and 11 personnel respectively. The Rams in particular employ 11 personnel on close to 90% of plays from 2022-2024 according to [Sūmer Sports](https://sumersports.com/teams/offensive/personnel-tendency/?personnel=11). So it makes sense that while they have a lower overall explosive play rate out of these personnel groupings, but the distribution is really tight. In 2023 George Kittle gave an interview with Pardon My Take where he outlines the core of what makes the Shanahan offense tick. 

<iframe width="560" height="315" src="https://www.youtube.com/embed/OcTPL8l_eUw?si=R5XbRzpVWsAyMvQQ&amp;start=3328" title="Kittle Explains Why The Scheme is So Effective" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

While you can make the argument that all four play callers are just different flavors of the West Coast offense, Payton in particular tends to mix in more personnel versatility. So much so that in 2019 former Bucs coach Dirk Koetter claimed that the Saints had show [28 different personnel groupings](https://www.nfl.com/news/keys-to-sean-mcvay-s-sean-payton-s-differing-offensive-methods-0ap3000001011448).^[Undoubtedly this is a mixture of how many TEs and RBs are on the field but also who is on the field.] We can see this born out in the model where, for most of, Mcvay's carrer he has primarily employed 12 personnel whereas Payton tends to use a mixture of personnel groupings. 

```{r}

just_seans = make_marginal_lift |>
    filter(coach  %in% c('Sean Payton', 'Sean McVay'))


ggplot(just_seans, aes(x = marginal_lift, y = personnel_nice, color = personnel_nice, fill = personnel_nice)) + 
    stat_halfeye(slab_alpha = 0.5) + 
    scale_color_met_d('Lakota') + 
    scale_fill_met_d('Lakota') + 
    labs(color = NULL, fill = NULL, y = NULL) + 
    facet_wrap(vars(coach), scales = 'free') +
    theme_minimal() +
    theme(legend.position = 'none')
...
[Ollie Connolly details](https://www.readoptional.com/p/the-art-of-play-sequencing-and-how) some of what makes the Payton offense tick. Philosophically one of the big divergences is where all the details show up. The Shanahan/McVay systems tend to employ very similar rules and are not neccessarily overly complicated. The bevy of details that lead to explosive plays tend to show up in what they are doing in other parts of the play. Whereas Payton is less wedded to particular personnel groupings and spends lots of time on protection plans. Part of this is because he wants to buy time for the routes to develop so his QB can hit them at the top of the drop.

We can also likely explain some of the discrepencies between in explosive play rate just due to overall team strength. If we examine the HSGP for Andy Reid we see that the Chiefs offense has been pretty explosive over his tenure. However, we start to see some volatility as they trade pieces away or as the offensive roster starts to degrade as we get closer to the 2026 season.  
...
reid_mask = raw_data_pd['off_play_caller'] == 'Andy Reid'
reid_obs = np.where(reid_mask)[0]

time_vals = raw_data_pd.loc[reid_mask, 'time_decimal'].values

obs_rate =  (raw_data['n_explosive'].to_numpy()[reid_mask] / 
            raw_data['total_plays'].to_numpy()[reid_mask])

mu_samples = az.extract(idata, group = 'posterior', var_names=['mu']).values[reid_obs, :]
...
axs.scatter(
    time_vals, obs_rate, color = 'black', alpha = 0.4, s = 15
)

axs.set(xlabel = 'time', ylabel = 'Observed Explosive Play Rate', title = "Andy Reid's Explosive Play Rate")
```
