...
The next thing that I changed was just getting rid of the week to week HSGP.^[That is an excercise left to the reader.] The most important part is to define the mixture part. 
...
Here we are going to try to learn whether each coach is in near zero play regime. We could really make this complicated by adding more and more covariates to our `w_coach_logit` param. However, just using the baseline for each coach gets us pretty close. The posterior predicitive follows our observed data a lot better. 
...
Instead of trying to tune this model within an inch of its life it is probably just going to be easier to build a mixture model where we are learning explosive play rate for passes and rushing separtely. 